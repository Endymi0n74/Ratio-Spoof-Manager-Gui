...
- **Release v2.0.0 de l'interface** — 4 assets, tous produits par le workflow sauf le sidecar qui est repris tel quel (empreinte du moteur conservée, `7c44d541…`) :
  ...
  L'exécutable publié est celui de la CI (`95307424…`), donc **différent en octets du build local** (`1bc24e8f…`) : même source, mais toolchain/linkeur du runner. Le sidecar dans `binaries/` est trouvé automatiquement (`engine_search_dirs` cherche à côté de l'exe puis dans `binaries/`).
