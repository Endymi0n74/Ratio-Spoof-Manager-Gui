...
- **GitHub :** <https://github.com/Endymi0n74/ratio-spoof-manager-tauri> — **renommé le 13/09/2026** (il s'appelait `Ratio_Spoof_Gui`), branche unique `master`. Release **[v2.0.0](https://github.com/Endymi0n74/ratio-spoof-manager-tauri/releases/tag/v2.0.0)** publiée le 13/09/2026 : paquet portable Windows (`ratio-spoof-manager.exe` + sidecar + `SHA256SUMS.txt`).
...
- **Release automatisée :** `.github/workflows/release.yml` (runner `windows-latest`) enchaîne `npm ci` → `engine:fetch` → `npm run tauri build -- --no-bundle` → assemblage du paquet portable → publication des 4 assets : `ratio-spoof-manager-v<version>-windows-x64-portable.zip` (exe + `binaries/ratio-spoof-x86_64-pc-windows-msvc.exe` + `LISEZ-MOI.txt` + `SHA256SUMS.txt` interne), `ratio-spoof-manager.exe`, le sidecar, et un `SHA256SUMS.txt` global. Le workflow **refuse** un tag dont la version diffère de `package.json`, et remplace les assets existants (`gh release upload --clobber`) au lieu d'échouer. Le mode d'emploi placé dans l'archive est versionné (`packaging/LISEZ-MOI.txt`), les empreintes sont écrites en LF (compatibles `sha256sum -c`). Conséquence assumée : **`package-lock.json` est désormais versionné** (`npm ci` et le cache npm de `setup-node` l'exigent).
...
- **Release v2.0.0 de l'interface** : exécutable + sidecar (asset `v1.9.1` du moteur, repris tel quel : digests GitHub identiques, `7c44d541…`) + `SHA256SUMS.txt`, puis le paquet portable construit par le workflow. Le paquet vit dans un dossier nommé `ratio-spoof-manager-v<version>-windows-x64` avec le sidecar dans `binaries/` — l'application le trouve à côté d'elle (`engine_search_dirs`) ; sinon « Chemin personnalisé ».
