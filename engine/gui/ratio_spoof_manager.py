...
class ModernRatioSpoofManager:
    ...
    def _launch(self):
        ...
        try:
            ...
            platform_options = (
                # CREATE_NEW_PROCESS_GROUP is required so we can later send
                # CTRL_BREAK_EVENT and let the engine shut down gracefully
                # (announce "stopped" to the tracker) instead of being
                # hard-killed by TerminateProcess.
    ...
    def _stop_process(self):
        ...
        if process and process.poll() is None:
            if IS_WINDOWS:
                # terminate() maps to TerminateProcess on Windows, which kills
                # the engine instantly and never lets it announce "stopped"
                # to the tracker. CTRL_BREAK_EVENT is delivered like a signal
                # instead, so the engine's own signal handler can run its
                # graceful-shutdown announce first.
                try:
            ...
            self._append_log("\nArrêt demandé…\n")
            threading.Thread(target=self._force_kill_if_stuck, args=(process,), daemon=True).start()

    def _force_kill_if_stuck(self, process):
        """Fallback: if the engine hasn't exited on its own after a grace
        period (e.g. stuck retrying an unreachable tracker), force-kill it
        so the user always has a way to actually stop it from the GUI."""
        time.sleep(STOP_GRACE_PERIOD_SECONDS)
        if process.poll() is None:
            try:
                ...
                self.root.after(0, self._append_log,
                                 "\nLe moteur ne répondait pas, arrêt forcé.\n")
            except OSError:
                pass
